    ...
    , ui(new Ui::MainWindow)
{
    ...
    mServer->setMaxPendingConnections(5);
    connect(mServer,SIGNAL(newConnection()),this,SLOT(TcpNewConnect()));
    mServer->listen(QHostAddress::Any,1234);
...
{
    mSocket = mServer->nextPendingConnection();//此函数无法让mSocket工作在另外一个线程中
    if(mSocket)
    {
//        nsocket->setSocketDescriptor(mSocket->socketDescriptor());
//        QThread *nthread = new QThread();
//        nsocket->moveToThread(nthread);
//        connect(nsocket,SIGNAL(SigTcpUi(QByteArray)),this,SLOT(setda(QByteArray)));//数据传递函数
//        nthread->start();
        connect(mSocket,SIGNAL(readyRead()),this,SLOT(writeda()));
    }
    qDebug()<<"主线线程："<<QThread::currentThreadId();
}

void MainWindow::writeda()
{
    QByteArray ba = "";
    while(ba != "/")
    {
    ...
    while(ba != "/")
    {
    password=password+ba;
    ba=mSocket->read(1);
    }
    ui->textEdit_1->setText(name);
    ui->textEdit_2->setText(password);
    denglu(name,password);
}

void MainWindow::setda(QByteArray ba)
{
    ui->textEdit_1->append(ba); //QByteArray 类型数据要用append
    qDebug()<<"回到线程："<<QThread::currentThreadId();
}

void MainWindow::denglu(QByteArray name,QByteArray password)
{
  ...
  * **/
      ...
      ;
       ...
       query.bindValue(":name","qqq");
       query.bindValue(":dev_id","7");
